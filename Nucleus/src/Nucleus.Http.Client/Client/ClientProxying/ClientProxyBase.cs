...
using Nucleus.Http.Client.Authentication;
...
namespace Nucleus.Http.Client.ClientProxying
{
    public class ClientProxyBase<TService> : ITransientDependency
    {
        ...
        protected IOptions<NucleusCorrelationIdOptions> NucleusCorrelationIdOptions => LazyServiceProvider.LazyGetRequiredService<IOptions<NucleusCorrelationIdOptions>>();
        ...
        protected IOptions<NucleusHttpClientOptions> ClientOptions => LazyServiceProvider.LazyGetRequiredService<IOptions<NucleusHttpClientOptions>>();
        ...
        protected virtual ClientProxyRequestContext BuildHttpProxyClientProxyContext(string methodName, ClientProxyRequestTypeValue arguments = null)
        {
            ...
            if (action == null)
            {
                throw new NucleusException($"The API description of the {typeof(TService).FullName}.{methodName} method was not found!");
            }
        ...
        {
            var clientConfig = ClientOptions.Value.HttpClientProxies.GetOrDefault(requestContext.ServiceType) ?? throw new NucleusException($"Could not get HttpClientProxyConfig for {requestContext.ServiceType.FullName}.");
            ...
            {
                throw new NucleusRemoteCallException($"An error occurred during the NUCLEUS remote HTTP request. ({ex.Message}) See the inner exception for details.", ex);
            }
        ...
        protected virtual async Task<string> GetConfiguredApiVersionAsync(ClientProxyRequestContext requestContext)
        {
            var clientConfig = ClientOptions.Value.HttpClientProxies.GetOrDefault(requestContext.ServiceType)
                               ?? throw new NucleusException($"Could not get DynamicHttpClientProxyConfig for {requestContext.ServiceType.FullName}.");
        ...
        {
            if (response.Headers.Contains(NucleusHttpConsts.NucleusErrorFormat))
            {
                ...
                {
                    throw new NucleusRemoteCallException(
                ...
                throw new NucleusRemoteCallException(errorResponse.Error)
                {
                    HttpStatusCode = (int) response.StatusCode
                };
            }
            ...
            ApiVersionInfo apiVersion)
        {
            ...
            //CorrelationId
            requestMessage.Headers.Add(NucleusCorrelationIdOptions.Value.HttpHeaderName, CorrelationIdProvider.Get());

            //TenantId
            if (CurrentTenant.Id.HasValue)
            {
                //TODO: Use NucleusAspNetCoreMultiTenancyOptions to get the key
