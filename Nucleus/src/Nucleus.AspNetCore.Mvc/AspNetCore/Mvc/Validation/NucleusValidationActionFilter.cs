...
{
    public class NucleusValidationActionFilter : IAsyncActionFilter, ITransientDependency
    {
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            ...
            if (!context.GetRequiredService<IOptions<NucleusAspNetCoreMvcOptions>>().Value.AutoModelValidation)
            {
                await next();
                return;
            }
