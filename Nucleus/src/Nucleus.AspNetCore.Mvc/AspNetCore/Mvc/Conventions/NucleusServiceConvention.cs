...
{
    public class NucleusServiceConvention : INucleusServiceConvention, ITransientDependency
    {
        public ILogger<NucleusServiceConvention> Logger { get; set; }

        protected NucleusAspNetCoreMvcOptions Options { get; }
        protected IConventionalRouteBuilder ConventionalRouteBuilder { get; }

        public NucleusServiceConvention(
            ...
            IConventionalRouteBuilder conventionalRouteBuilder)
        {
            ...
            Logger = NullLogger<NucleusServiceConvention>.Instance;
        }
        ...
        protected virtual void ConfigureSelector(string rootPath, string controllerName, ActionModel action, [CanBeNull] ConventionalControllerSetting configuration)
        {
            ...
            {
                AddNucleusServiceSelector(rootPath, controllerName, action, configuration);
            }
            else
            {
        ...
        protected virtual void AddNucleusServiceSelector(string rootPath, string controllerName, ActionModel action, [CanBeNull] ConventionalControllerSetting configuration)
        {
            var httpMethod = SelectHttpMethod(action, configuration);

            var nucleusServiceSelectorModel = new SelectorModel
            {
                AttributeRouteModel = CreateNucleusServiceAttributeRouteModel(rootPath, controllerName, action, httpMethod, configuration),
                ActionConstraints = { new HttpMethodActionConstraint(new[] { httpMethod }) }
            };

            action.Selectors.Add(nucleusServiceSelectorModel);
        }
        ...
        protected virtual void NormalizeSelectorRoutes(string rootPath, string controllerName, ActionModel action, [CanBeNull] ConventionalControllerSetting configuration)
        {
            foreach (var selector in action.Selectors)
            {
                ...
                {
                    selector.AttributeRouteModel = CreateNucleusServiceAttributeRouteModel(rootPath, controllerName, action, httpMethod, configuration);
                }
        ...
        protected virtual AttributeRouteModel CreateNucleusServiceAttributeRouteModel(string rootPath, string controllerName, ActionModel action, string httpMethod, [CanBeNull] ConventionalControllerSetting configuration)
        {
