...
namespace Nucleus.EventBus.Distributed
{
    public abstract class DistributedEventBusBase : EventBusBase, IDistributedEventBus, ISupportsEventBoxes
    {
        ...
        protected NucleusDistributedEventBusOptions NucleusDistributedEventBusOptions { get; }

        protected DistributedEventBusBase(
            IServiceScopeFactory serviceScopeFactory,
            ICurrentTenant currentTenant,
            IUnitOfWorkManager unitOfWorkManager,
            IOptions<NucleusDistributedEventBusOptions> nucleusDistributedEventBusOptions,
            ...
            unitOfWorkManager)
        {
            ...
            Clock = clock;
            NucleusDistributedEventBusOptions = nucleusDistributedEventBusOptions.Value;
        }
        ...
        private async Task<bool> AddToOutboxAsync(Type eventType, object eventData)
        {
            ...
            foreach (var outboxConfig in NucleusDistributedEventBusOptions.Outboxes.Values)
            {
        ...
        protected async Task<bool> AddToInboxAsync(
            ...
            byte[] eventBytes)
        {
            if (NucleusDistributedEventBusOptions.Inboxes.Count <= 0)
            {
                return false;
            }
            ...
            {
                foreach (var inboxConfig in NucleusDistributedEventBusOptions.Inboxes.Values)
                {
