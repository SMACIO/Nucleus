...
namespace Nucleus.Features
{
    public static class FeatureCheckerExtensions
    {
        ...
        public static async Task CheckEnabledAsync(this IFeatureChecker featureChecker, string featureName)
        {
            if (!(await featureChecker.IsEnabledAsync(featureName)))
            {
                throw new NucleusAuthorizationException(code: NucleusFeatureErrorCodes.FeatureIsNotEnabled).WithData(
        ...
        public static async Task CheckEnabledAsync(this IFeatureChecker featureChecker, bool requiresAll, params string[] featureNames)
        {
            ...
            if (requiresAll)
            {
                foreach (var featureName in featureNames)
                {
                    if (!(await featureChecker.IsEnabledAsync(featureName)))
                    {
                        throw new NucleusAuthorizationException(code: NucleusFeatureErrorCodes.AllOfTheseFeaturesMustBeEnabled)
            ...
            else
            {
                ...
                throw new NucleusAuthorizationException(code: NucleusFeatureErrorCodes.AtLeastOneOfTheseFeaturesMustBeEnabled)
                    .WithData("FeatureNames", string.Join(", ", featureNames));
            }
        }
    }
