...
namespace Nucleus.Http.Client.ClientProxying
{
    public class ClientProxyUrlBuilder : ITransientDependency
    {
        ...
        protected NucleusHttpClientProxyingOptions HttpClientProxyingOptions { get; }

        public ClientProxyUrlBuilder(IServiceScopeFactory serviceScopeFactory, IOptions<NucleusHttpClientProxyingOptions> httpClientProxyingOptions)
        {
        ...
        protected virtual Task ReplacePathVariablesAsync(StringBuilder urlBuilder, IList<ParameterApiDescriptionModel> actionParameters, IReadOnlyDictionary<string, object> methodArguments, ApiVersionInfo apiVersion)
        {
            ...
            foreach (var pathParameter in pathParameters.Where(p => p.Name != "apiVersion")) //TODO: Constant!
            {
                ...
                if (value == null)
                {
                    ...
                    {
                        throw new NucleusException($"Missing path parameter value for {pathParameter.Name} ({pathParameter.NameOnMethod})");
                    }
                }
                else
                {
