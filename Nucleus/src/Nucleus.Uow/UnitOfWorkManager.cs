...
namespace Nucleus.Uow
{
    public class UnitOfWorkManager : IUnitOfWorkManager, ISingletonDependency
    {
        ...
        public IUnitOfWork Begin(NucleusUnitOfWorkOptions options, bool requiresNew = false)
        {
        ...
        public void BeginReserved(string reservationName, NucleusUnitOfWorkOptions options)
        {
            if (!TryBeginReserved(reservationName, options))
            {
                throw new NucleusException($"Could not find a reserved unit of work with reservation name: {reservationName}");
            }
        }

        public bool TryBeginReserved(string reservationName, NucleusUnitOfWorkOptions options)
        {
            Check.NotNull(reservationName, nameof(reservationName));
